...
class CSLEConvert
{
    ...
    public static object FromJson(string ScriptTypeName, MyJson.JsonNode_Object data)
    {
        ...
        foreach (var m in stype.members)
        {
...

            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine("发现不能处理的类型:" + ScriptTypeName + ":" + m.Key + ":" + m.Value.type.ToString());
                }
            }
    ...
    public static MyJson.JsonNode_Object ToJson(object ScriptObject)
    {
        ...
        foreach (var m in stype.members)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine("发现不能处理的类型:" + stype.Name + ":" + m.Key + ":" + m.Value.type.ToString());
            }
        }
        return obj;
    }
